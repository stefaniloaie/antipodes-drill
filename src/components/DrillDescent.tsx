...
export default function DrillDescent({ onDone }: { onDone: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden select-none" style={{ background: "#050608" }}>
      {/* Earth cross-section */}
      <div
        className="absolute inset-x-0 h-[400vh] earth-gradient will-change-transform"
        style={{ opacity: emerging ? 0.15 : 0.6, transform: `translateY(${-p * 200}vh)`, transition: "opacity 1.5s ease" }}
      />
      <div
        className="absolute inset-x-0 h-[400vh] earth-gradient will-change-transform"
        style={{ opacity: emerging ? 0.15 : 0.6, transform: `translateY(${-p * 200}vh) scaleY(-1)`, marginTop: "400vh", transition: "opacity 1.5s ease" }}
      />
      <div
      ...
      {coreReached && (
        <div
          ...
          style={{ zIndex: 20, animation: "core-breach-in 0.35s cubic-bezier(0.22,1,0.36,1) forwards" }}
        >
          <div style={{
      ...
      {/* Motion streaks */}
      <div className="streaks absolute inset-0" style={{ opacity: emerging ? 0.04 : 0.06 + speed * 0.3, zIndex: 1 }} />
      ...
      <div
        className="pointer-events-none absolute inset-0"
        style={{ background: "radial-gradient(ellipse at 50% 50%, transparent 25%, rgba(0,0,0,0.85) 100%)", zIndex: 6 }}
      />

      {/* Letterbox */}
      <div className="pointer-events-none absolute inset-x-0 top-0 h-[8vh] bg-[#050608]" style={{ zIndex: 7 }} />
      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-[8vh] bg-[#050608]" style={{ zIndex: 7 }} />

      {/* TOP HUD */}
      <div className="absolute inset-x-0 top-0 px-6 py-5 sm:px-12 sm:py-6" style={{ zIndex: 8 }}>
        <div className="flex flex-wrap items-end gap-x-10 gap-y-3">
          <div>
            <p style={{ fontFamily: "var(--font-mono)", fontSize: "0.58rem", letterSpacing: "0.25em", color: "rgba(255,255,255,0.28)", textTransform: "uppercase" }}>DEPTH</p>
            <div style={{ fontFamily: "var(--font-mono)", fontSize: "clamp(2rem,5vw,3.8rem)", lineHeight: 1, color: "#fff", letterSpacing: "-0.02em" }}>
              {Math.round(depth).toLocaleString()}
              <span style={{ fontSize: "0.9rem", marginLeft: "0.35rem", color: "rgba(255,255,255,0.35)" }}>km</span>
            </div>
          </div>
          <div>
            <p style={{ fontFamily: "var(--font-mono)", fontSize: "0.58rem", letterSpacing: "0.25em", color: "rgba(255,255,255,0.28)", textTransform: "uppercase" }}>TEMP</p>
            <div style={{ fontFamily: "var(--font-mono)", fontSize: "clamp(1.3rem,3.2vw,2.4rem)", lineHeight: 1, color: cfg.color, letterSpacing: "-0.02em", transition: "color 0.8s ease" }}>
              {temp.toLocaleString()}
              <span style={{ fontSize: "0.8rem", marginLeft: "0.3rem", color: "rgba(255,255,255,0.28)" }}>°C</span>
            </div>
          </div>
          <div className="hidden sm:block">
            <p style={{ fontFamily: "var(--font-mono)", fontSize: "0.58rem", letterSpacing: "0.25em", color: "rgba(255,255,255,0.28)", textTransform: "uppercase" }}>SPEED</p>
            <div style={{ fontFamily: "var(--font-mono)", fontSize: "clamp(1.1rem,2.5vw,1.9rem)", lineHeight: 1, color: "rgba(255,255,255,0.55)" }}>
              {Math.round(speed * 2400).toLocaleString()}
              <span style={{ fontSize: "0.7rem", marginLeft: "0.3rem", color: "rgba(255,255,255,0.22)" }}>km/h</span>
            </div>
          </div>
          <div className="hidden md:block">
            <p style={{ fontFamily: "var(--font-mono)", fontSize: "0.58rem", letterSpacing: "0.25em", color: "rgba(255,255,255,0.28)", textTransform: "uppercase" }}>PRESSURE</p>
            <div style={{ fontFamily: "var(--font-mono)", fontSize: "clamp(1.1rem,2.5vw,1.9rem)", lineHeight: 1, color: "rgba(255,255,255,0.55)" }}>
              {Math.round((realDepth / EARTH_RADIUS_KM) * 360).toLocaleString()}
              <span style={{ fontSize: "0.7rem", marginLeft: "0.3rem", color: "rgba(255,255,255,0.22)" }}>GPa</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="absolute inset-x-0 bottom-0 px-6 py-5 sm:px-12 sm:py-6" style={{ zIndex: 8 }}>
        ...
        >
          <div style={{ height: 3, background: cfg.color, width: `${layerPct}%`, transition: "width 0.12s linear, background 0.7s ease" }} />
          <div style={{ padding: "14px 18px 16px" }}>
            <div className="flex items-start justify-between gap-4">
              <div>
                <p style={{ fontFamily: "var(--font-mono)", fontSize: "0.6rem", letterSpacing: "0.2em", color: cfg.color, textTransform: "uppercase", transition: "color 0.7s ease" }}>
                  {cfg.label} · {layer.temp}
                </p>
                <h2 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(1rem,2.4vw,1.45rem)", color: "#fff", marginTop: 3 }}>
                  {layer.name}
                </h2>
                <p style={{ fontSize: "0.78rem", color: "rgba(255,255,255,0.42)", marginTop: 5, lineHeight: 1.6 }}>
                  {layer.note}
                </p>
              </div>
              <svg width="48" height="48" viewBox="0 0 48 48" className="mt-1 shrink-0">
                ...
                />
                <text x="24" y="28" textAnchor="middle" fontSize="11" fill="rgba(255,255,255,0.65)" fontFamily="monospace">
                  {layerPct}%
                </text>
              </svg>
            </div>
        ...
        <div className="mt-4 flex items-center gap-3">
          <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.58rem", color: "rgba(255,255,255,0.22)", width: 28 }}>
            {Math.round(p * 100)}%
          </span>
          ...
          <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.58rem", color: "rgba(255,255,255,0.22)", width: 54, textAlign: "right" }}>
